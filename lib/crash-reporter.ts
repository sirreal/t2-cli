...
CrashReporter.off = function () {
	return Preferences.write(CRASH_REPORTER_PREFERENCE, 'off').catch((error) => {
		// do nothing
...
CrashReporter.on = function () {
	return Preferences.write(CRASH_REPORTER_PREFERENCE, 'on').catch((error) => {
		// do nothing
		// do not crash the crash reporter :)
...
CrashReporter.sanitize = function (input) {
	return CrashReporter.sanitize.redactions.reduce(
		(output, redaction) => redaction(output),
		input,
	);
};
...
CrashReporter.sanitize.redactions = [
	(stack) => {
		...
		if (index !== -1) {
			stack = stack.replace(
				new RegExp(escape(__dirname.slice(0, index)), 'g'),
				'',
			);
		}
	...
	(argv) =>
		argv.replace(/(-p.*,\s)(.*)/, (_, keep) => `${keep}----redacted----`),
];

CrashReporter.prompt = function () {
	return new Promise((resolve) => {
		Preferences.read(CRASH_REPORTER_PROMPT, 'true').then((value) => {
			if (value === 'false') {
				...
				prompt.then((selection) => {
					if (selection.selected === false) {
						...
						return Preferences.write(CRASH_REPORTER_PROMPT, 'false').then(
							() => {
								resolve(true);
							},
						);
...
CrashReporter.submit = function (report, opts) {
	...
	return CrashReporter.prompt().then((success) => {
		if (success) {
			return Preferences.read(CRASH_REPORTER_PREFERENCE, 'on')
              ...
              `;
						...
						var argv = CrashReporter.sanitize(
							opts.argv || process.argv.slice(2).join(', '),
						);
						var stack = CrashReporter.sanitize(report.stack || String(report));

						return CrashReporter.post(labels, stack, argv).then(
							(fingerprint) => {
								if (!silent) {
									log.info(
										`Crash Reported: ${CRASH_REPORTER_BASE_URL}/crashes?fingerprint=${fingerprint}`,
									);
								}
							},
						);
...
CrashReporter.post = function (labels, crash, argv) {
	...
	return new Promise((resolve, reject) => {
		remote
			.ifReachable(remote.CRASH_REPORTER_HOSTNAME)
			.then(() => {
				request.post(
					{
						url,
...
CrashReporter.status = () => {
	return Preferences.load().then((prefs) => {
		log.info(
			`Crash Reporter is ${prefs[CRASH_REPORTER_PREFERENCE].toUpperCase()}`,
		);
	});
};

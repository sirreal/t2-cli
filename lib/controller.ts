...
Tessel.list = function (opts) {
	...
	return new Promise((resolve, reject) => {
		...
		var noTessels = opts.authorized ? responses.noAuth : responses.auth;

		// When a Tessel is found
		seeker.on('tessel', (tessel) => {
			var note = '';
			...
			if (
				tessel.connection.connectionType === 'LAN' &&
			...
			// Print out details...
			log.basic(
				`\t${tessel.connection.connectionType}\t${tessel.name}\t${note}`,
			);
		});
		...
		seeker.once('end', () => {
			...
			// If we have only one Tessel or two Tessels with the same name (just USB and LAN)
				...
				foundTessels.length === 1 ||
				(foundTessels.length === 2 &&
					foundTessels[0].name === foundTessels[1].name)
			) {
				// Close all opened connections and resolve
				controller
					.closeTesselConnections(foundTessels)
			...
			// Otherwise
				...
				return controller
					...
					.then((tessel) => {
						...
						log.info(`Will default to ${tessel.displayName}.`);
					})
					.catch((error) => {
...
Tessel.get = function (options) {
	...
	return new Promise((resolve, reject) => {
		...
		const noTessels = options.authorized ? responses.noAuth : responses.auth;
									...
									return Menu.prompt({
										...
										prompt: {
											...
											choices: tessels.map((tessel, i) => {
												...
												const isAuthorized =
													isLAN && tessel.lanConnection.authorized;
												// show "(not authorized)" if there is a lanConnection
												/* istanbul ignore next */
												const authorization =
													isAuthorized || !!tessel.usbConnection
														? ''
														: '(not authorized)';
												const display = `\t${tessel.connection.connectionType}\t${tessel.displayName}\t${authorization}`;
										...
										if (!tessel) {
											return controller
												.closeTesselConnections(tessels)
												.then(() =>
			...
			if (tessel) {
				...
				controller
					.closeTesselConnections(
						tessels.filter((closable) => closable !== tessel),
					)
					.then(() => resolve(tessel));
			} else {
		...
		return (
			controller
				...
				.then((tessel) => {
					...
					var sigintPromise = new Promise((resolve) =>
		...
		async.each(
			...
			(tessel, done) => {
				...
				if (
					...
					if (
						tessel.connection.connectionType === 'USB' &&
						tessel.connection.closed
					) {
						done();
					} else {
						// Close the connection
						return tessel.close().then(done, done);
					}
				} else {
					done();
				}
...
const NAME_OPTION_PRIORITY = 1;
const ENV_OPTION_PRIORITY = 2;
const USB_CONN_PRIORITY = 3;
const LAN_CONN_PRIORITY = 4;

const hasOtherConnections = (kind, entries, exclusions) => {
	return entries
		.filter(({ tessel }) => !exclusions.includes(tessel))
		.some(({ priority }) => priority === kind);
};
...
controller.runHeuristics = ({ name }, tessels) => {
	return Promise.all(
		tessels.map((tessel) => {
			...
			if (name && name === tessel.name) {
				priority = NAME_OPTION_PRIORITY;
			}
			...
			if (
				...
				priority = ENV_OPTION_PRIORITY;
			}

			// Tessel has a USB connection
			if (!priority && tessel.usbConnection) {
				priority = USB_CONN_PRIORITY;
			}

			// Tessel has a LAN connection
			if (!priority && tessel.lanConnection) {
				priority = LAN_CONN_PRIORITY;
			}
			return Promise.resolve({
				tessel,
				priority,
		...
		for (const { tessel, priority } of entries) {
			// If this is a name option or environment variable option,
			// then it can be immediately returned as the obviously
			// intended Tessel.
			if (
				priority === NAME_OPTION_PRIORITY ||
				priority === ENV_OPTION_PRIORITY
			...
			if (priority === USB_CONN_PRIORITY) {
				if (hasOtherConnections(USB_CONN_PRIORITY, entries, [tessel])) {
					// If there are other entries to look at that may
					// available, then allow the search to continue uninterrupted.
					if (hasOtherConnections(LAN_CONN_PRIORITY, entries, [tessel])) {
						continue;
					}
					// When there is more than one Tessel connected to this user's
					// computer via USB port, and they have specified no other
					// identifiers, it is too ambigiuous to guess which Tessel
					// the user intends to target with the given command, so we
					// exit with a useful error.
					return Promise.reject(
						new controller.HeuristicAmbiguityError(USB_CONN_PRIORITY),
					);
				}

				// If there is no ambiguity, assume the user wants to target the
				// lone USB connected Tessel. There is no need to review the rest of
				// the list for possible LAN connected boards, because those are
				// lower priority.
				return tessel;
			}

			if (priority === LAN_CONN_PRIORITY) {
				if (hasOtherConnections(LAN_CONN_PRIORITY, entries, [tessel])) {
					// If there are other entries to look at with different connections
					// available, then allow the search to continue uninterrupted.
					if (hasOtherConnections(USB_CONN_PRIORITY, entries, [tessel])) {
						continue;
					}
					// When there are no Tessels connected via USB, but there are many
					// Tessels accessible via LAN, and the users has not specified any
					// other identifiers, it is too ambigiuous to guess which Tessel
					// the user intends to target with the given command, so we
					// exit with a useful error.
					return Promise.reject(
						new controller.HeuristicAmbiguityError(LAN_CONN_PRIORITY),
					);
				}

				// Before returning this Tessel, check that there isn't a possible better
				// match later in the list of potentional Tessels
				if (!hasOtherConnections(USB_CONN_PRIORITY, entries, [tessel])) {
					// If there is no ambiguity, assume the user wants to target the
					// lone LAN connected Tessel.
					return tessel;
				}
			}
		}
		...
		return entries[0].tessel;
	});
};

controller.HeuristicAmbiguityError = function (message) {
	Error.captureStackTrace(this, this.constructor);
	this.name = this.constructor.name;
	this.message = `It is unclear which device should be operated upon. (Priority Code: ${message})`;
};

util.inherits(controller.HeuristicAmbiguityError, Error);

controller.provision = function (opts) {
	opts = opts || {};
	return new Promise((resolve, reject) => {
		...
		return controller.standardTesselCommand(opts, (tessel) => {
			...
			return tessel.provision(opts);
		});
	});
};

controller.restore = function (options) {
	options.usb = true;
	options.lan = false;
...
controller.eraseScript = function (options) {
	options.authorized = true;
	return controller.standardTesselCommand(options, (tessel) =>
		tessel.eraseScript(options, false),
	);
};

controller.rename = function (options) {
	options = options || {};
	options.authorized = true;
	...
	return new Promise((resolve, reject) => {
		if (!options.reset && !options.newName) {
			...
			if (!options.reset && !Tessel.isValidName(options.newName)) {
				reject(
					`Invalid name: ${options.newName}. The name must be a valid hostname string. See http://en.wikipedia.org/wiki/Hostname#Restrictions_on_valid_host_names.`,
				);
			} else {
				resolve();
			}
		...
		return controller.standardTesselCommand(options, (tessel) => {
			...
			log.info(
				`Renaming ${tessel.displayName} to ${colors.magenta(options.newName)}`,
			);
...
controller.printAvailableNetworks = function (options) {
	...
	return controller.standardTesselCommand(options, (tessel) => {
		...
		return tessel.findAvailableNetworks().then((networks) => {
			log.info(
				`Found ${networks.length} network${networks.length !== 1 ? 's' : ''} visible to ${tessel.displayName}:`,
			);
	...
	return controller.standardTesselCommand(options, (tessel) => {
		return tessel
			...
			.then((network) => {
				...
				var ips = network.ips
					.filter((item) => /inet/.exec(item))
					.map((line) => line.split(' '))
					.reduce((a, b) => a.concat(b))
					.filter((item) => /addr/.exec(item))
					.map((chunk) => chunk.split(':')[1])
					.filter((addr) => addr.length);

				log.info(`SSID: ${network.ssid}`);
				ips.forEach((ip) => log.info(`IP Address: ${ip}`));
				log.info(
					`Signal Strength: (${network.quality}/${network.quality_max})`,
				);
...
controller.connectToNetwork = function (options) {
	...
	return new Promise(function (resolve, reject) {
		if (!ssid) {
			return reject(
				'Invalid credentials: must set SSID with the -n or --ssid option.',
			);
		}

		if (security && !password) {
			return reject(
				'Invalid credentials: must set a password with the -p or --password option.',
			);
		}
		...
		if (security) {
			if (securityOptions.indexOf(security) === -1) {
				return reject(
					`"${security}" is not a valid security option. Please choose one of the following: ${securityOptions.join(', ')}`,
				);
			} else if (security.match(/wpa2?/)) {
				return reject(
					`"${security}" security is not yet implemented. Please see this issue for more details -> https://github.com/tessel/t2-cli/issues/803`,
				);
			}
		}
		resolve();
		...
		return controller.standardTesselCommand(options, (tessel) =>
			tessel.connectToNetwork(options),
		);
	});
};
...
controller.createAccessPoint = function (options) {
	...
	return new Promise((resolve, reject) => {
		...
		if (security && securityOptions.indexOf(security) === -1) {
			reject(
				`${security} is not a valid security option. Please choose on of the following: ${securityOptions.join(', ')}`,
			);
		}
		...
		if (security === 'wep') {
			...
			if (length !== 10 && length !== 26 && length !== 58) {
				return reject(
					'Invalid passphrase: WEP keys must be 10, 26, or 58 digits long for 64-, 128-, and 256-bit WEP.',
				);
			}
		}
		...
		if (security === 'psk' || security === 'psk2') {
			...
			if (!asciiRegex.test(password) && !hexTest) {
				return reject(
					'Invalid passphrase: WPA/WPA2-PSK passkeys must be 8-63 ASCII characters, or 64 hexadecimal digits.',
				);
			}
		}
		resolve();
		...
		return controller.standardTesselCommand(options, (tessel) =>
			tessel.createAccessPoint(options),
		);
	});
};
...
controller.getAccessPointInfo = function (options) {
	options.authorized = true;
	return controller.standardTesselCommand(options, (tessel) => {
		return tessel.getAccessPointInfo().then((ap) => {
			if (ap.ssid) {
				...
				log.info(
					`${tessel.displayName} is not configured as an access point (run "t2 ap --help" to learn more)`,
				);
			}
		});
	});
...
controller.root = function (options) {
	...
	if (options.usb) {
		options.usb = false;
		log.warn(
			'You are trying to connect to Tessel via USB, but this command only works with Wifi.',
		);
	...
	return controller.standardTesselCommand(options, function (tessel) {
		log.info(
			'Starting SSH Session on Tessel. Type "exit" at the prompt to end.',
		);
		return new Promise(function (resolve, reject) {
...
controller.updateWithRemoteBuilds = function (opts, tessel) {
	return new Promise((resolve, reject) => {
		...
		if (!tessel.usbConnection) {
			return reject(
				'Must have Tessel connected over USB to complete update. Aborting update.',
			);
		}
			...
			.then((builds) => {
				...
				if (!opts.force) {
					...
					versionFromSHA = new Promise((resolve, reject) => {
						...
						return (
							tessel
								.fetchCurrentBuildInfo()
								// Once we have the current SHA, provide the version
								.then((currentSHA) =>
									resolve(updates.findBuild(builds, 'sha', currentSHA)),
								)
								.catch((error) => {
									// If there was an error because the version file doesn't exist
									if (error.message.includes('No such file or directory')) {
										log.warn(
											`Could not find firmware version on ${tessel.displayName}`,
										);
				...
				return versionFromSHA.then(function (currentVersionInfo) {
					...
					if (version && build) {
						...
						return controller.updateTesselWithVersion(
							opts,
							tessel,
						...
						if (
							!opts.force &&
							semver.gte(currentVersionInfo.version, verifiedVersion)
						) {
							// If it's not, close the Tessel connection and print the error message
							var message = `${tessel.displayName} is already on the latest firmware version (${currentVersionInfo.version}). You can force an update with "t2 update --force".`;
							...
							log.info(
								`Updating ${tessel.displayName} to latest version (${verifiedVersion})...`,
							);
...
controller.updateTesselWithVersion = function (
	opts,
	tessel,
	currentVersion,
	build,
	...
	return updates.fetchBuild(build).then((image) => {
		...
		return (
			tessel
				...
				.then(() => {
					if (!opts.force) {
						log.info(
							`Updated ${tessel.displayName} from ${currentVersion} to ${build.version}`,
						);
					} else {
						log.info(
							`Force updated ${tessel.displayName} to version ${build.version}`,
						);
					}
					// We need to force this thing to exit the process since it will
					// otherwise just hang as it tries to close the newly flashed Tessel.
					process.exit();
				})
		);
	});

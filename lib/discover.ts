...
export class TesselSeeker extends Emitter {
	constructor() {
		super();
		this.lanScan = null;
		this.usbScan = null;
	...
	start(opts = {}) {
		...
		var connectionHandler = (conn) => {
			...
			var opening = tessel.connection
				.open(opts.altSetting)
				.then(() => {
					if (opts.authorized !== undefined) {
						if (tessel.connection.authorized !== opts.authorized) {
							debug(
								`Kicking ${conn.host.slice(0, -6)} due to authorized filter: ${opts.authorized}`,
							);

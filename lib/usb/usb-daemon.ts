...
class USBDaemon {
	static _singletonInstance: null | USBDaemon = null;
	...
	constructor() {
		...
		if (USBDaemon._singletonInstance) {
			return USBDaemon._singletonInstance;
		} else {
			USBDaemon._singletonInstance = this;
		}
	}
	...
	openProcess(connection, callback) {
		...
		if (!entry) {
			return (
				...
				callback(
					new Error(
						'This USB Connection was never registered with the daemon...',
					),
				)
			);
		}
	...
	closeProcess(entry, proc, callback) {
  ...
  them to the process as necessary */
	...
	_nextID() {
		...
		if (usedIDs.length > MAX_PROCESS_ID) {
			throw new Error(
				'Attempt to spawn more than the maximum number of processes over USB',
			);
		} else {

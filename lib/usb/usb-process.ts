// Built-ins
import type { USBDaemon } from './usb-daemon';
...
class USBProcess extends EventEmitter {
	...
	control: RemoteWritableStream;
	stdin: RemoteWritableStream;
	stdout: RemoteReadableStream;
	stderr: RemoteReadableStream;
	...
	constructor(id, daemon) {
		...
		this.control = new RemoteWritableStream(
			this,
			daemon,
			protocol.controlWrite,
			protocol.controlClose,
		);
		this.stdin = new RemoteWritableStream(
			this,
			daemon,
			protocol.stdinWrite,
			protocol.stdinClose,
	...
	close() {
		...
		this.daemon.write(protocol.closeProcess(this.id));
	...
	kill(signal) {
		...
		if (this.waitForClose) {
			...
			this.daemon.write(protocol.killProcess(this.id, signal));
		} else {
...
class RemoteWritableStream extends stream.Writable {
	...
	_packetize(chunkSizes, inputBuffer) {

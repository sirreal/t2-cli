...
function download(url) {
  return fetch(url).then(response => {
    ...
    var nodeStream = Readable.fromWeb(response.body).pipe(progressStream);
    return nodeStream;
  });
}
  ...
  return remote.ifReachable(remote.BUILDS_HOSTNAME).then(() => {
    return downloadString(`${url}.sha256`)
      ...
      .then((check) => {
        ...
        return download(url).then(stream => extractTools(checksumVerify, path.basename(url), stream));
      });
  });
};
    ...
    .then((rustv) => {
      ...
      return downloadString(url + '.sha256')
        ...
        .then(check => {
          ...
          return download(url).then(stream => extractRustlib(checksumVerify, path.basename(url), stream, rustv));
        });
    });
};

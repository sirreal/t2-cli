...
exportables.preRun = function (tessel, opts) {
	// jshint ignore:line
	return tessel.simpleExec(
		commands.chmod('+x', `${Tessel.REMOTE_RUN_PATH}${opts.resolvedEntryPoint}`),
	);
};
...
exportables.remoteRustCompilation = (opts) => {
	...
	return new Promise((resolve, reject) => {
		...
		remote
			.ifReachable(destination.hostname)
			.then(() => {
				// Set up the request
                ...
                You received an invalid JSON response from the cross-compilation server.
                    ${Buffer.concat(buffers).toString()}`);
							...
							if (result.binary === null) {
								...
								return reject(
									new Error(
										'Neither binary nor error returned by cross compilation server.',
									),
								);
							}

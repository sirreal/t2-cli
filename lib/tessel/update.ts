...
*/
	Tessel.prototype.fetchCurrentBuildInfo = function () {
		...
		return this.simpleExec(commands.readFile(remoteVersioningFile)).then(
	...
	Tessel.prototype.updateOpenWRT = function (opts, image) {
		...
		return this.fixOldUpdateScripts()
				...
				return new Promise((resolve, reject) => {
					...
					this.connection.exec(
						...
						(err, remoteProc) => {
							...
							log.info(
								`Transferring image of size ${(image.length / 1e6).toFixed(2)} MB. This will take 2-4 minutes...`,
							);
            ...
            */
							var updateCompleteTimeout = setTimeout(
								resolve,
								Tessel.openWRTUpdateTime,
							);
	...
	Tessel.prototype.fixOldUpdateScripts = function () {
		return (
			new Promise((resolve, reject) => {
				...
				fs.readFile(
					path.join(__dirname, '../../resources/openwrt/common.sh'),
					(err, contents) => {
						if (err) {
							return reject(err);
						} else {
							...
							this.connection.exec(
								commands.openStdinToFile('/lib/upgrade/common_tmp.sh'),
								(err, remoteProcess) => {
									if (err) {
										return reject(err);
									} else {

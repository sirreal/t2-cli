...
export function registerMethods(Tessel) {
	...
	Tessel.prototype.getName = function () {
		...
		return this.simpleExec(commands.getHostname()).then((name) => {
	...
	Tessel.prototype.setName = function (name) {
		return new Promise((resolve, reject) => {
			...
			return (
				this.simpleExec(commands.setHostname(name))
					.then(() => {
					...
					.then(() => {
						...
						this.connection.exec(
							commands.openStdinToFile('/proc/sys/kernel/hostname'),
							(err, remoteProc) => {
								if (err) {
									return reject(err);
								} else {
					...
					.then(() => {
						return this.resetMDNS().then(resolve);
					})
					.catch(reject)
			);
		});
	...
	Tessel.prototype.rename = function (options) {
		return new Promise((resolve, reject) => {
			...
			if (options.reset) {
				...
				this.getName().then((oldName) => {
					return this.setName(options.newName).then(() => {
						log.info(
							`Changed name of device ${colors.magenta(oldName)} to ${colors.magenta(options.newName)}`,
						);
						resolve();
					});
				});
	...
	Tessel.prototype.getMACAddress = function () {
		return this.simpleExec(commands.getInterface('eth0')).then(
			function fetchedInterface(interfaceInfo) {

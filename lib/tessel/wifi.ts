...
export function registerMethods(Tessel) {
			...
			.then((resultsJSON) => {
				try {
					...
					if (network.ssid === undefined) {
						var msg = `${this.displayName} is not connected to Wi-Fi (run "t2 wifi -l" to see available networks)`;
				...
				return this.simpleExec(commands.getIPAddress()).then(
					function (ipResults) {
	...
	function safeQualityExprEvaluation(expr) {
		...
		const nexpr = +expr;
		const isNumber =
			parsed === null && typeof nexpr === 'number' && !Number.isNaN(nexpr);
	...
	Tessel.prototype.connectToNetwork = function (options) {
		...
		if (password && security) {
			...
			if (options.showpassword) {
				...
				let rpass = /(^[a-z0-9])(.*?)([a-z0-9])$/gi;
				passToDisplay = password.replace(
					rpass,
					(w, f, m, l) => `${f}${'*'.repeat(m.length)}${l}`,
				);
			}
		...
		const setNetworkSecurity = () =>
			this.simpleExec(commands.setNetworkEncryption(security));
		const turnWifiOn = () =>
			this.setWiFiState({
				enable,
			});
	...
	Tessel.prototype.resetMDNS = function () {
		return this.simpleExec(commands.callMDNSDaemon('restart')).then(() =>
			this.simpleExec(commands.callTesselMDNS('restart')),
		);
	};
	...
	Tessel.prototype.setWiFiState = function (state) {
		...
		if (typeof state.enable === 'undefined') {
			return Promise.reject(
				new Error('Missing Wifi State: property "enable" not provided.'),
			);
		}
		...
		return new Promise((resolve, reject) => {
			return this.simpleExec(commands.turnOnWifi(state.enable))
				...
				.then(() => {
					return this.simpleExec(commands.reconnectWifi()).then((result) => {
						...
						if (result.includes("'radio0' is disabled")) {
							return this.simpleExec(commands.turnRadioOn())
								.then(() =>
									this.simpleExec(commands.commitWirelessCredentials()),
								)
								...
								this.receive(remoteProcess, (error, result) => {
									...
									if (result.toString().includes('signal')) {
										...
										if (tries) {
											...
											settle(
												'Unable to verify connection. Please ensure you have entered the correct network credentials.',
											);
										}
									}
								});

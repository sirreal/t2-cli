...
function commitAndClose(tessel, status, resolve, reject) {
	...
	return tessel
		.simpleExec(commands.commitWirelessCredentials())
		.then(reconnectWifi)
...
export function registerMethods(Tessel) {
	Tessel.prototype.enableAccessPoint = function () {
		...
		return new Promise((resolve, reject) => {
			return this.getAccessPointInfo()
				.then((info) => {
					if (info.ssid) {
						return this.simpleExec(commands.turnAccessPointOn()).then(() => {
						...
						reject(
							`${this.displayName} is not configured as an access point (run "t2 ap --help" to learn more)`,
						);
					}
				})
				.catch(reject);
	...
	Tessel.prototype.disableAccessPoint = function () {
		...
		return new Promise((resolve, reject) => {
			return this.simpleExec(commands.turnAccessPointOff()).then(() =>
				commitAndClose(this, status, resolve, reject),
			);
		});
	};
		...
		var setupAccessPoint = () => {
			...
			var setup = () => {
				if (password) {
					...
					return setSSID(ssid).then(setAccessPointSecurity);
				}
			};

			return setup().then(turnAccessPointOn).then(commitAndClosePromise);
		};

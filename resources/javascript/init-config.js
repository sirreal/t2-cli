...
module.exports = {
			...
			: function (cb) {
					fs.readdir('node_modules', function (er, dir) {
						...
						dir.forEach(function (d) {
							...
							if (
								d.match(/^(expresso|mocha|tap|coffee-script|coco|streamline)$/)
							)
								return next();
							fs.readFile(
								...
								function (er, p) {
									...
									if (p._requiredBy && p._requiredBy.includes('#USER')) {
										deps[d] = '~' + p.version;
									}
	...
	scripts: (function () {
		...
		if (!s.test || s.test === notest) {
			...
			else s.test = prompt('test command', tx);
		}

...
exports['USB.Connection.prototype.open'] = {
	setUp(done) {
		...
		this.getStringDescriptor = this.sandbox.spy(
			this.usbConnection.device,
			'getStringDescriptor',
		);
	...
	setAltSettingFails(test) {
		...
		this.connectionAltSetting = this.sandbox
			.stub(this.usbConnection, 'setAltSetting')
			.returns(Promise.reject('bad usb things'));
	...
	multipleOpens(test) {
		this.daemonDeregister = this.sandbox
			.stub(Daemon, 'deregister')
			.callsFake((val, cb) => {
...
exports['USB.Connection.prototype.end'] = {
	...
	resolveWaitOnClose(test) {
		...
		this.closeFunc = this.sandbox
			.stub(this.usbConnection, '_close')
			.callsFake((cb) => {
				waited = true;
				setImmediate(cb);
			});

...
exports['Tessel.prototype.restore'] = {
	setUp(done) {
		...
		this.status = this.sandbox
			.stub(restore, 'status')
			.callsFake(() => Promise.resolve(0));
		this.fetchRestore = this.sandbox
			.stub(updates, 'fetchRestore')
			.callsFake(() => {
	...
	restoreWithValidateDeviceId(test) {
		...
		this.validateDeviceId = this.sandbox
			.stub(restore, 'validateDeviceId')
			.callsFake(() => Promise.resolve());
		this.transaction = this.sandbox
			.stub(restore, 'transaction')
			.callsFake(() => Promise.resolve());

		this.tessel.restore({}).then(() => {
	...
	restoreFetchImages(test) {
		...
		this.flash = this.sandbox
			.stub(restore, 'flash')
			.callsFake(() => Promise.resolve());
		this.transaction = this.sandbox
			.stub(restore, 'transaction')
			.callsFake((usb, bytesOrCommand) => {
...
exports['restore.*'] = {
	...
	validateDeviceIdSuccess(test) {
		...
		this.transaction = this.sandbox
			.stub(restore, 'transaction')
			.callsFake(() => Promise.resolve(Buffer.from([0x01, 0x02, 0x19])));

		restore.validateDeviceId({}).then(() => {
	...
	validateDeviceIdFailure(test) {
		...
		this.transaction = this.sandbox
			.stub(restore, 'transaction')
			.callsFake(() => Promise.resolve(Buffer.from([0x00, 0x00, 0x00])));

		restore.validateDeviceId({}).catch((error) => {
			test.equal(
				error.message,
				'Invalid Device ID (Flash Memory Communication Error)',
			);
			test.done();
		});
	},
...
exports['restore.transaction'] = {
	...
	transactionAcceptsCommandNumber(test) {
		...
		restore.transaction(this.usb, 0xff).then(() => {
			test.equal(
				this.usb.epOut.transfer.lastCall.args[0].equals(this.expectedBuffer),
				true,
			);
...
exports['restore.bulkEraseFlash'] = {
	setUp(done) {
		...
		this.transaction = this.sandbox
			.stub(restore, 'transaction')
			.callsFake(() => Promise.resolve());
		this.waitTransactionComplete = this.sandbox
			.stub(restore, 'waitTransactionComplete')
			.callsFake(() => Promise.resolve());
...
exports['restore.flash'] = {
	setUp(done) {
		...
		this.status = this.sandbox
			.stub(restore, 'status')
			.callsFake(() => Promise.resolve(0));
		this.enableWrite = this.sandbox
			.stub(restore, 'enableWrite')
			.callsFake(() => Promise.resolve());
		this.bulkEraseFlash = this.sandbox
			.stub(restore, 'bulkEraseFlash')
			.callsFake(() => Promise.resolve());
		this.write = this.sandbox
			.stub(restore, 'write')
			.callsFake(() => Promise.resolve());
	...
	completeRestoreCallSteps(test) {
		...
		this.controlTransfer = this.sandbox.stub(
			this.tessel.usbConnection.device,
			'controlTransfer',
		);
...
exports['restore.write'] = {
	setUp(done) {
		...
		this.writePage = this.sandbox
			.stub(restore, 'writePage')
			.callsFake(() => Promise.resolve());
		this.waitTransactionComplete = this.sandbox
			.stub(restore, 'waitTransactionComplete')
			.callsFake(() => Promise.resolve());

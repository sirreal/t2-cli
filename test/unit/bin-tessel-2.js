...
exports['Tessel (t2: makeCommand)'] = {
	any(test) {
		...
		t2.makeCommand('any').callback(function () {
...
exports['Tessel (t2: restart)'] = {
	setUp(done) {
		...
		this.restart = this.sandbox
			.stub(controller, 'restart')
			.returns(Promise.resolve());
		this.closeFailedCommand = this.sandbox.spy(t2, 'closeFailedCommand');
		this.closeSuccessfulCommand = this.sandbox.stub(
			t2,
	...
	invalidType(test) {
		...
		setImmediate(() => {
			...
			test.equal(this.closeFailedCommand.callCount, 1);
			test.equal(
				this.closeFailedCommand.lastCall.args[0].trim(),
				'--type Invalid',
			);
			test.equal(this.processExit.lastCall.args[0], 1);
			test.done();
	...
	entryPointFallbackToPrevious(test) {
		...
		this.sandbox
			.stub(Preferences, 'read')
			.returns(Promise.resolve('previous.js'));
	...
	entryPointFallbackNoPrevious(test) {
		...
		resolved.then(() => {
			test.equal(
				this.closeFailedCommand.lastCall.args[0],
...
exports['Tessel (t2: update)'] = {
	setUp(done) {
		...
		this.error = this.sandbox.stub(log, 'error');
		this.warn = this.sandbox.stub(log, 'warn');
		this.printAvailableUpdates = this.sandbox
			.stub(controller, 'printAvailableUpdates')
			.returns(Promise.resolve());
		this.update = this.sandbox
			.stub(controller, 'update')
			.returns(Promise.resolve());
		this.closeFailedCommand = this.sandbox.spy(t2, 'closeFailedCommand');
		this.processExit = this.sandbox.stub(process, 'exit');
...
exports['Tessel (t2: restore)'] = {
	setUp(done) {
		...
		this.error = this.sandbox.stub(log, 'error');
		this.warn = this.sandbox.stub(log, 'warn');
		this.restore = this.sandbox
			.stub(controller, 'restore')
			.returns(Promise.resolve());
		this.closeFailedCommand = this.sandbox.spy(t2, 'closeFailedCommand');
		this.processExit = this.sandbox.stub(process, 'exit');
...
exports['Tessel (t2: version)'] = {
	setUp(done) {
		...
		this.error = this.sandbox.stub(log, 'error');
		this.warn = this.sandbox.stub(log, 'warn');
		this.envVersions = this.sandbox
			.stub(controller, 'envVersions')
			.returns(Promise.resolve());
		this.closeFailedCommand = this.sandbox.spy(t2, 'closeFailedCommand');
		this.processExit = this.sandbox.stub(process, 'exit');
...
exports['Tessel (t2: key)'] = {
	setUp(done) {
		...
		this.error = this.sandbox.stub(log, 'error');
		this.warn = this.sandbox.stub(log, 'warn');
		this.setupLocal = this.sandbox
			.stub(controller, 'setupLocal')
			.returns(Promise.resolve());
		this.closeFailedCommand = this.sandbox.spy(t2, 'closeFailedCommand');
		this.processExit = this.sandbox.stub(process, 'exit');
...
exports['Tessel (t2: ap)'] = {
	setUp(done) {
		...
		this.enableAccessPoint = this.sandbox
			.stub(controller, 'enableAccessPoint')
			.returns(Promise.resolve());
		this.disableAccessPoint = this.sandbox
			.stub(controller, 'disableAccessPoint')
			.returns(Promise.resolve());
		this.createAccessPoint = this.sandbox
			.stub(controller, 'createAccessPoint')
			.returns(Promise.resolve());
		this.getAccessPointInfo = this.sandbox
			.stub(controller, 'getAccessPointInfo')
			.returns(Promise.resolve());
		this.closeFailedCommand = this.sandbox.spy(t2, 'closeFailedCommand');
		this.processExit = this.sandbox.stub(process, 'exit');
...
exports['Tessel (t2: provision)'] = {
	setUp(done) {
		...
		this.warn = this.sandbox.stub(log, 'warn');
		this.info = this.sandbox.stub(log, 'info');
		this.provision = this.sandbox
			.stub(controller, 'provision')
			.returns(Promise.resolve());
		this.closeFailedCommand = this.sandbox.spy(t2, 'closeFailedCommand');
		this.processExit = this.sandbox.stub(process, 'exit');
...
exports['Tessel (t2: reboot)'] = {
	setUp(done) {
		...
		this.warn = this.sandbox.stub(log, 'warn');
		this.info = this.sandbox.stub(log, 'info');
		this.reboot = this.sandbox
			.stub(controller, 'reboot')
			.returns(Promise.resolve());
		this.closeFailedCommand = this.sandbox.spy(t2, 'closeFailedCommand');
		this.processExit = this.sandbox.stub(process, 'exit');
...
exports['Tessel (t2: wifi)'] = {
	setUp(done) {
		...
		this.warn = this.sandbox.stub(log, 'warn');
		this.info = this.sandbox.stub(log, 'info');
		this.printAvailableNetworks = this.sandbox
			.stub(controller, 'printAvailableNetworks')
			.returns(Promise.resolve());
		this.connectToNetwork = this.sandbox
			.stub(controller, 'connectToNetwork')
			.returns(Promise.resolve());
		this.getWifiInfo = this.sandbox
			.stub(controller, 'getWifiInfo')
			.returns(Promise.resolve());
...
exports['Tessel (t2: root)'] = {
	setUp(done) {
		...
		this.warn = this.sandbox.stub(log, 'warn');
		this.info = this.sandbox.stub(log, 'info');
		this.root = this.sandbox
			.stub(controller, 'root')
			.returns(Promise.resolve());
		this.successfulCommand = this.sandbox.stub(t2, 'closeSuccessfulCommand');
		done();
	},
...
exports['Tessel (t2: crash-reporter)'] = {
	setUp(done) {
		...
		this.crOn = this.sandbox
			.stub(CrashReporter, 'on')
			.returns(Promise.resolve());
		this.crOff = this.sandbox
			.stub(CrashReporter, 'off')
			.returns(Promise.resolve());
		this.crPost = this.sandbox
			.stub(CrashReporter, 'post')
			.returns(Promise.resolve());
		this.crStatus = this.sandbox
			.stub(CrashReporter, 'status')
			.returns(Promise.resolve());
		this.crSubmit = this.sandbox
			.stub(CrashReporter, 'submit')
			.returns(Promise.resolve());
		this.crTest = this.sandbox
			.stub(CrashReporter, 'test')
			.returns(Promise.resolve());
...
exports['Tessel (init)'] = {
	...
	'defaults to --lang=js'(test) {
		...
		var resolve = Promise.resolve();
		this.createNewProject = this.sandbox
			.stub(controller, 'createNewProject')
			.returns(resolve);
...
exports['Tessel (t2: installer-*)'] = {
	setUp(done) {
		...
		this.drivers = this.sandbox
			.stub(installer, 'drivers')
			.returns(Promise.resolve());

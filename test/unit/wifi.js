...
exports['Tessel.prototype.findAvailableNetworks'] = {
	setUp(done) {
		...
		this.findAvailableNetworks = this.sandbox.spy(
			Tessel.prototype,
	...
	noNetworks(test) {
		...
		this.tessel.findAvailableNetworks().then(function (networks) {
...
`;
		...
		this.tessel.findAvailableNetworks().then((found) => {
	...
	rejectUnknownError(test) {
			...
			if (command.toString() === commands.scanWiFi().join(' ')) {
				setImmediate(() => {
					this.tessel._rps.stderr.emit(
						'data',
						Buffer.from('No such wireless device'),
					);
					this.tessel._rps.emit('close');
				});
			} else {
		...
		this.tessel
			...
			.catch((error) => {
				test.equal(
					error.message,
					'Unable to find wireless networks while wifi is disabled. Run "t2 wifi --on" before trying again.',
				);
				test.done();
			});
	},
...
module.exports['Tessel.prototype.connectToNetwork'] = {
	setUp(done) {
		...
		this.connectToNetwork = this.sandbox.spy(
			Tessel.prototype,
			'connectToNetwork',
		);
		...
		this.setNetworkEncryption = this.sandbox.spy(
			commands,
			'setNetworkEncryption',
		);
		this.commitWirelessCredentials = this.sandbox.spy(
			commands,
		...
		this.tessel._rps.on('control', (command) => {
			if (command.toString() === 'ubus call iwinfo info {"device":"wlan0"}') {
				...
				setImmediate(() => {
					...
					this.tessel._rps.stderr.emit(
						'data',
						Buffer.from('Unable to connect to the network.'),
					);
					this.tessel._rps.emit('close');
				});
			} else {
			...
			if (command.toString() === 'ubus call iwinfo info {"device":"wlan0"}') {
				...
				setImmediate(() => {
					...
					this.tessel._rps.stdout.emit(
						'data',
						Buffer.from('{"frequency": 2422,"txpower": 20}'),
					);
					this.tessel._rps.stdout.removeAllListeners();
	...
	batchedResponse(test) {
		...
		this.tessel._rps.on('control', (command) => {
			if (command.toString() === 'ubus call iwinfo info {"device":"wlan0"}') {
				...
				setImmediate(() => {
					...
					this.tessel._rps.stdout.emit(
						'data',
						Buffer.from('do not have success yet'),
					);
		...
		this.tessel
			...
			.catch((error) => {
				test.ok(
					false,
					'Should not have received an error with batched response',
					error,
				);
				test.done();
			});
...
module.exports['Tessel.prototype.setWifiState'] = {
	...
	setWifiStateMissingState(test) {
		...
		this.tessel.setWiFiState().catch((error) => {
	...
	setWifiStateTruthy(test) {
		...
		this.tessel
			...
			.then(() => {
				...
				test.deepEqual(this.turnOnWifi.lastCall.returnValue, [
					'uci',
					'set',
	...
	setWifiStateFalsy(test) {
		...
		this.tessel._rps.on('control', (command) => {
			if (
				command.toString() === 'ubus call iwinfo info {"device":"wlan0"}' &&
				state.enable
			) {
				// Write to stdout so it completes as expected
				// Wrap in setImmediate to make sure listener is set up before emitting
		...
		this.tessel._rps.on('control', (command) => {
			if (
				...
				setImmediate(() => {
					this.tessel._rps.stdout.emit(
						'data',
						Buffer.from(`'radio0' is disabled`),
					);
					this.tessel._rps.emit('close');
				});
			} else {
			...
			.catch((error) => {
				test.ok(
					error
						.toString()
						.includes(
							'not connected to Wi-Fi (run "t2 wifi -l" to see available networks)',
						),
				);
				test.ok(this.getWifiInfo.callCount, 1);
				test.done();
	...
	getWifiInfoDisabled(test) {
		...
		this.tessel
			...
			.then(() => {
				test.ok(
					false,
					'fetching wifi info with disabled interface should reject.',
				);
				test.done();
			})
			.catch((error) => {

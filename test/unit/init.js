...
exports['init.createNewProject()'] = {
	setUp(done) {
		...
		this.lang = {
			js: {
				generateProject: this.sandbox
					.stub(init.js, 'generateProject')
					.returns(Promise.resolve()),
			},
			py: {
				generateProject: this.sandbox
					.stub(init.py, 'generateProject')
					.returns(Promise.resolve()),
			},
			rs: {
				generateProject: this.sandbox
					.stub(init.rs, 'generateProject')
					.returns(Promise.resolve()),
			},
		};
	...
	resolveLanguage: {
		...
		unrecognized(test) {
			...
			this.resolveLanguage = this.sandbox
				.stub(init, 'resolveLanguage')
				.returns(null);
			...
			init
				...
				.catch((error) => {
					test.equal(
						error.toString(),
						'Error: Unrecognized language selection.',
					);
					test.equal(this.resolveLanguage.callCount, 1);
					test.done();
				});
...
exports['init --lang=rust'] = {
	setUp: function (done) {
		...
		this.copy = this.sandbox
			.stub(fs, 'copy')
			.callsFake((source, target, callback) => {
				callback();
			});
		done();
	},
	...
	cargoVerifySucceed: function (test) {
		...
		this.createSampleProgram = this.sandbox
			.stub(init.rs, 'createSampleProgram')
			.returns(Promise.resolve());
		...
		init
			...
			.catch(() => {
				test.ok(
					false,
					'a rejection should not be served if cargo is installed',
				);
			});
	},
	...
	cargoVerifyFail: function (test) {
		...
		init
			...
			.then(() => {
				test.ok(
					false,
					'a rejection should be served if cargo is not installed',
				);
			})
			.catch((err) => {
				// Ensure exec was called just once
	...
	'src/main.rs created': function (test) {
		...
		this.exists = this.sandbox
			.stub(fs, 'exists')
			.callsFake((filename, callback) => {
				callback(false);
			});

		this.mkdir = this.sandbox
			.stub(fs, 'mkdir')
			.callsFake((dirname, callback) => {
				callback(false);
			});
		...
		init.rs
			...
			.then(() => {
				...
				test.equal(
					this.copy.firstCall.args[0].endsWith(
						path.normalize('/resources/rust/Cargo.toml'),
					),
					true,
				);
				test.equal(
					this.copy.firstCall.args[1].endsWith(path.normalize('Cargo.toml')),
					true,
				);
				test.equal(
					this.copy.lastCall.args[0].endsWith(
						path.normalize('/resources/rust/main.rs'),
					),
					true,
				);
				test.equal(
					this.copy.lastCall.args[1].endsWith(path.normalize('/src/main.rs')),
					true,
				);
				test.done();
			})
...
exports['init --lang=javascript'] = {
	setUp(done) {
		...
		this.copy = this.sandbox
			.stub(fs, 'copy')
			.callsFake((src, dest, callback) => {
				callback();
			});

		const projectDependencyPath = path.normalize(
	...
	allOperations(test) {
		...
		operations.forEach((operation) => {
			...
			this[operation.name] = this.sandbox
				.stub(init.js, operation.name)
				.callsFake(() => Promise.resolve('{"a":1}'));
		});

		init.js.generateProject({}).then(() => {
			test.ok(true);
			operations.forEach((operation) => {
	...
	readPackageJsonSuccess(test) {
		...
		this.readFile = this.sandbox
			.stub(fs, 'readFile')
			.callsFake((file, encoding, callback) => {
				callback(null, '{}');
			});
	...
	npmInstallWithDepsSuccess(test) {
		...
		this.loadNpm = this.sandbox
			.stub(init.js, 'loadNpm')
			.returns(Promise.resolve(npm));

		init.js.npmInstall(dependencies).then(() => {
	...
	npmInstallWithDepsFailure(test) {
		...
		init.js.npmInstall(dependencies).catch((error) => {
	...
	createSampleProgram(test) {
		...
		init.js.createSampleProgram().then(() => {
			test.equal(this.copy.callCount, 1);
			test.equal(
				this.copy.lastCall.args[0].endsWith(
					path.normalize('resources/javascript/index.js'),
				),
				true,
			);
			test.equal(
				this.copy.lastCall.args[1].endsWith(path.normalize('index.js')),
				true,
			);
			test.done();
		});
	...
	createTesselinclude(test) {
		...
		init.js.createTesselinclude().then(() => {
			test.equal(this.copy.callCount, 1);
			test.equal(
				this.copy.lastCall.args[0].endsWith(
					path.normalize('resources/javascript/.tesselinclude'),
				),
				true,
			);
			test.equal(
				this.copy.lastCall.args[1].endsWith(path.normalize('.tesselinclude')),
				true,
			);
			test.done();
		});

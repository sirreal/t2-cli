...
exports['CrashReporter.on'] = {
	setUp(done) {
		...
		this.pWrite = this.sandbox
			.stub(Preferences, 'write')
...
exports['CrashReporter.submit'] = {
	setUp(done) {
		...
		this.pRead = this.sandbox
			.stub(Preferences, 'read')
			.returns(Promise.resolve('on'));
		this.pWrite = this.sandbox
			.stub(Preferences, 'write')
			.returns(Promise.resolve());
		this.crPrompt = this.sandbox
			.stub(CrashReporter, 'prompt')
			.returns(Promise.resolve(true));
		this.crPost = this.sandbox.spy(CrashReporter, 'post');
		this.crSanitize = this.sandbox.spy(CrashReporter, 'sanitize');
		this.request = this.sandbox
			.stub(request, 'post')
			.callsFake((opts, handler) => {
				return handler(null, {}, '{}');
			});
		done();
	},
	...
	submit(test) {
		...
		CrashReporter.submit(report).then(() => {
			...
			test.ok(
				args[0].includes('CLI version'),
				'Label CLI version should be present',
			);
			test.ok(
				args[0].includes('Node version'),
				'Label Node version should be present',
			);
			test.ok(
				args[0].includes('OS platform'),
				'Label OS platform should be present',
			);
			test.ok(
				args[0].includes('OS release'),
				'Label OS release should be present',
			);
	...
	sanitizes(test) {
		...
		this.crPost = this.sandbox
			...
			.returns(Promise.resolve());
		var error = new Error(
			`This happened at ${__dirname}. Line 1 in ${__filename}`,
		);
	...
	sanitizesArgv1(test) {
		...
		CrashReporter.submit(new Error('This happened'), {
			...
			.then(() => {
				test.equal(
					this.crPost.lastCall.args[2],
					'wifi, -n, somessid, -password, ----redacted----',
				);
				test.done();
			})
			.catch((error) => {
	...
	sanitizesArgv3(test) {
		...
		CrashReporter.submit(new Error('This happened'), {
			...
			.then(() => {
				test.equal(
					this.crPost.lastCall.args[2],
					'ap, -n, somessid, -password, ----redacted----',
				);
				test.done();
			})
			.catch((error) => {
...
exports['CrashReporter.sanitize'] = {
	...
	home(test) {
		...
		var error = new Error(
			`permission denied, open '${path.join(os.homedir(), '.config')}'`,
		);
		...
		this.request = this.sandbox
			...
			.callsFake((opts, handler) => {
				return handler(
					null,
					{},
					'{"error": "Strange things are afoot at the Circle-K"}',
				);
			});

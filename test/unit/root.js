...
exports['tessel.root'] = {
	setUp: function (done) {
		...
		this.standardTesselCommand = this.sandbox.spy(
			controller,
			'standardTesselCommand',
		);
	...
	spawnNotCalledNoTessels: function (test) {
		...
		this.finishWithError = this.sandbox
			.stub(t2, 'closeFailedCommand')
			.callsFake(() => {
	...
	spawnCalledWithLANTessel: function (test) {
		...
		// Root should call closeFailed Command if no Tessels were found
		this.finishWithSuccess = this.sandbox
			.stub(t2, 'closeSuccessfulCommand')
			.callsFake(() => {
				// We called standardTesselCommand to fetch the Tessel

	...
	successful(test) {
		...
		installer.drivers().then(() => {
			...
			if (isLinux) {
				...
				test.equal(
					this.copySync.lastCall.args[0].endsWith('resources/85-tessel.rules'),
					true,
				);
				test.equal(
					this.copySync.lastCall.args[1],
...
exports['installer.homedir'] = {
	setUp(done) {
		...
		this.ensureDir = this.sandbox
			.stub(fs, 'ensureDir')
			.callsFake((target, callback) => callback(null));
		this.ensureFile = this.sandbox
			.stub(fs, 'ensureFile')
			.callsFake((target, callback) => callback(null));
		this.readJson = this.sandbox
			.stub(fs, 'readJson')
			.callsFake((target, callback) => callback(null));
		this.outputJson = this.sandbox
			.stub(fs, 'outputJson')
			.callsFake((target, data, callback) => callback(null));
	...
	successfulInitialization(test) {
		...
		installer.homedir().then(() => {
			...
			test.equal(
				this.ensureDir.lastCall.args[0],
				path.join(osenv.home(), '.tessel'),
			);
			test.equal(
				this.ensureFile.lastCall.args[0],
				path.join(osenv.home(), '.tessel', 'preferences.json'),
			);
			test.equal(
				this.readJson.lastCall.args[0],
				path.join(osenv.home(), '.tessel', 'preferences.json'),
			);
			test.equal(
				this.outputJson.lastCall.args[0],
				path.join(osenv.home(), '.tessel', 'preferences.json'),
			);

...
exports['commands.app'] = {
	...
	disable(test) {
		test.expect(1);
		test.deepEqual(commands.app.disable(), [
			'/etc/init.d/tessel-app',
			'disable',
		]);
		test.done();
	},
...
exports['commands.*'] = {
	...
	getWifiInfo(test) {
		test.expect(1);
		test.deepEqual(commands.getWifiInfo(), [
			'ubus',
			'call',
			'iwinfo',
			'info',
	...
	connectedNetworkStatus(test) {
		test.expect(1);
		test.deepEqual(commands.connectedNetworkStatus(), [
			'ubus',
			'call',
	...
	commitWirelessCredentials(test) {
		test.expect(1);
		test.deepEqual(commands.commitWirelessCredentials(), [
			'uci',
			'commit',
			'wireless',
		]);
	...
	getHostname(test) {
		test.expect(1);
		test.deepEqual(commands.getHostname(), [
			'uci',
			'get',
	...
	setLanNetwork(test) {
		test.expect(1);
		test.deepEqual(commands.setLanNetwork(), [
			'uci',
			'set',
	...
	setLanNetworkIfname(test) {
		test.expect(1);
		test.deepEqual(commands.setLanNetworkIfname(), [
			'uci',
			'set',
	...
	setLanNetworkProto(test) {
		test.expect(1);
		test.deepEqual(commands.setLanNetworkProto(), [
			'uci',
			'set',
	...
	setLanNetworkIP(test) {
		test.expect(1);
		test.deepEqual(commands.setLanNetworkIP(), [
			'uci',
			'set',
			'network.lan.ipaddr=192.168.1.101',
		]);
		test.done();
	},
	setLanNetworkNetmask(test) {
		test.expect(1);
		test.deepEqual(commands.setLanNetworkNetmask(), [
			'uci',
			'set',
	...
	getAccessPoint(test) {
		test.expect(1);
		test.deepEqual(commands.getAccessPoint(), [
			'uci',
			'get',
			'wireless.@wifi-iface[1]',
		]);
		test.done();
	},
	getAccessPointSSID(test) {
		test.expect(1);
		test.deepEqual(commands.getAccessPointSSID(), [
			'uci',
			'get',
	...
	setAccessPoint(test) {
		test.expect(1);
		test.deepEqual(commands.setAccessPoint(), [
			'uci',
			'add',
			'wireless',
			'wifi-iface',
	...
	setAccessPointDevice(test) {
		test.expect(1);
		test.deepEqual(commands.setAccessPointDevice(), [
			'uci',
			'set',
	...
	setAccessPointNetwork(test) {
		test.expect(1);
		test.deepEqual(commands.setAccessPointNetwork(), [
			'uci',
			'set',
	...
	setAccessPointMode(test) {
		test.expect(1);
		test.deepEqual(commands.setAccessPointMode(), [
			'uci',
			'set',
	...
	turnAccessPointOn(test) {
		test.expect(1);
		test.deepEqual(commands.turnAccessPointOn(), [
			'uci',
			'set',
	...
	turnAccessPointOff(test) {
		test.expect(1);
		test.deepEqual(commands.turnAccessPointOff(), [
			'uci',
			'set',
	...
	turnRadioOn(test) {
		test.expect(1);
		test.deepEqual(commands.turnRadioOn(), [
			'uci',
			'set',
	...
	getAccessPointConfig(test) {
		test.expect(1);
		test.deepEqual(commands.getAccessPointConfig(), [
			'uci',
			'show',
			'wireless.@wifi-iface[1]',
		]);
	...
	getAccessPointIP(test) {
		test.expect(1);
		test.deepEqual(commands.getAccessPointIP(), [
			'uci',
			'get',
			'network.lan.ipaddr',
		]);
	...
	reconnectDnsmasq(test) {
		test.expect(1);
		test.deepEqual(commands.reconnectDnsmasq(), [
			'/etc/init.d/dnsmasq',
			'restart',
		]);
		test.done();
	...
	reboot(test) {
		test.expect(1);
		test.deepEqual(commands.reboot(), [
			'sh',
			'-c',
			'echo 39 > /sys/class/gpio/export && echo out > /sys/class/gpio/gpio39/direction && echo 0 > /sys/class/gpio/gpio39/value && reboot',
		]);
		test.done();
	},
	readFile(test) {
	...
	deleteFolder(test) {
		test.expect(1);
		test.deepEqual(commands.deleteFolder('filepath'), [
			'rm',
			'-rf',
			'filepath',
		]);
	...
	createFolder(test) {
		test.expect(1);
		test.deepEqual(commands.createFolder('filepath'), [
			'mkdir',
			'-p',
			'filepath',
		]);
	...
	moveFolder(test) {
		test.expect(1);
		test.deepEqual(commands.moveFolder('source', 'destination'), [
			'mv',
			'source',
			'destination',
		]);
	...
	untarStdin(test) {
		test.expect(1);
		test.deepEqual(commands.untarStdin('filepath'), [
			'tar',
			'-x',
			'-C',
			'filepath',
	...
	appendStdinToFile(test) {
		test.expect(1);
		test.deepEqual(commands.appendStdinToFile('filepath'), [
			'tee',
			'-a',
			'filepath',
		]);
	...
	chmod(test) {
		test.expect(1);
		test.deepEqual(commands.chmod('+x', 'filepath'), [
			'chmod',
			'+x',
			'filepath',
		]);
	...
	setNetworkSSID(test) {
		test.expect(1);
		test.deepEqual(commands.setNetworkSSID('ssid'), [
			'uci',
			'set',
	...
	setNetworkPassword(test) {
		test.expect(1);
		test.deepEqual(commands.setNetworkPassword('key'), [
			'uci',
			'set',
	...
	setNetworkEncryption(test) {
		test.expect(1);
		test.deepEqual(commands.setNetworkEncryption('encryption'), [
			'uci',
			'set',
			'wireless.@wifi-iface[0].encryption=encryption',
		]);
		test.done();
	},
	turnOnWifi(test) {
		test.expect(2);
		test.deepEqual(commands.turnOnWifi(0), [
			'uci',
			'set',
			'wireless.@wifi-iface[0].disabled=1',
		]);
		test.deepEqual(commands.turnOnWifi(1), [
			'uci',
			'set',
	...
	ensureFileExists(test) {
		test.expect(1);
		test.deepEqual(commands.ensureFileExists('filepath'), [
			'touch',
			'filepath',
		]);
		test.done();
	...
	setHostname(test) {
		test.expect(1);
		test.deepEqual(commands.setHostname('hostname'), [
			'uci',
			'set',
	...
	getInterface(test) {
		test.expect(1);
		test.deepEqual(commands.getInterface('interfaceName'), [
			'ifconfig',
			'interfaceName',
		]);
		test.done();
	...
	callMDNSDaemon(test) {
		test.expect(1);
		test.deepEqual(commands.callMDNSDaemon('action'), [
			'/etc/init.d/mdnsd',
			'action',
		]);
		test.done();
	...
	callTesselMDNS(test) {
		test.expect(1);
		test.deepEqual(commands.callTesselMDNS('action'), [
			'/etc/init.d/tessel-mdns',
			'action',
		]);
		test.done();
	},
	...
	sysupgradeNoSaveConfig(test) {
		test.deepEqual(commands.sysupgradeNoSaveConfig('path'), [
			'sysupgrade',
			'-n',
			'path',
		]);
	...
	setAccessPointSSID(test) {
		test.expect(1);
		test.deepEqual(commands.setAccessPointSSID('ssid'), [
			'uci',
			'set',
			'wireless.@wifi-iface[1].ssid=ssid',
		]);
		test.done();
	},
	setAccessPointPassword(test) {
		test.expect(1);
		test.deepEqual(commands.setAccessPointPassword('password'), [
			'uci',
			'set',
	...
	setAccessPointSecurity(test) {
		test.expect(1);
		test.deepEqual(commands.setAccessPointSecurity('security'), [
			'uci',
			'set',
			'wireless.@wifi-iface[1].encryption=security',
		]);
		test.done();
	},
};

...
exports['provision.setDefaultKey'] = {
	...
	noKeyPath(test) {
		...
		provision
			...
			.then(() => {
				test.ok(
					false,
					'noKeyPath - should not pass because provision.setDefaultKey should return rejection',
				);
				test.done();
			})
			.catch(() => {
	...
	validAlternateKey(test) {
		...
		createKeyTestFolder((error) => {
			...
			async.parallel(
				[
					(next) => fs.writeFile(testPath + '.pub', publicKey, fileOpts, next),
					(next) => fs.writeFile(testPath, privateKey, fileOpts, next),
				],
				(error) => {
					if (error) {
						test.ok(false, error.message);
					...
					provision
						...
						.catch(() => {
							test.ok(
								false,
								'validAlternateKey - provision.setDefaultKey promize rejection',
							);
							test.done();
						});
				},

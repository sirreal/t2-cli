...
exports['TesselSeeker.prototype.start'] = {
	setUp(done) {
		...
		this.usbStartScan = this.sandbox
			.stub(usb, 'startScan')
			.callsFake(function () {
				return new FakeScanner();
			});
		this.lanStartScan = this.sandbox
			.stub(lan, 'startScan')
			.callsFake(function () {
				return new FakeScanner();
			});
		this.usbStopScan = this.sandbox
			.stub(usb, 'stopScan')
			.callsFake(function () {
				return null;
			});
		this.lanStopScan = this.sandbox
			.stub(lan, 'stopScan')
			.callsFake(function () {
				return null;
			});
...
exports['TesselSeeker Scan Time'] = {
	...
	oneAuthorizedLANPending(test) {
		...
		standardSeekerSetup(this.seeker, seekerOpts).then((found) => {
			// Make sure we only have the one Tessel we created
			test.equal(found.length, 1);

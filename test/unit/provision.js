...
exports['Tessel.isProvisioned()'] = {
	...
	isProvisionedTrue(test) {
		...
		test.equal(
			this.existsSync.firstCall.args[0],
			path.join(Tessel.LOCAL_AUTH_PATH, 'id_rsa'),
		);
		test.equal(
			this.existsSync.lastCall.args[0],
			path.join(Tessel.LOCAL_AUTH_PATH, 'id_rsa.pub'),
		);
...
exports['controller.provision'] = {
	setUp(done) {
		...
		this.writeFile = this.sandbox
			.stub(fs, 'writeFile')
			.callsFake((file, contents, options, callback) => {
				callback(null, callback);
			});

		this.readFile = this.sandbox
			.stub(fs, 'readFile')
			.callsFake((file, options, callback) => {
				callback(null, pubKey.trim());
			});

		this.provision = this.sandbox.spy(Tessel.prototype, 'provision');

		this.getName = this.sandbox
			.stub(Tessel.prototype, 'getName')
			.callsFake(() => {
		...
		this.exportKey = this.sandbox
			.stub(RSA.prototype, 'exportKey')
			.callsFake((_) => _);
		this.parseKey = this.sandbox.stub(sshpk, 'parseKey').callsFake((_) => _);
		this.RSA = this.sandbox
			.stub(global, 'RSA')
			.returns(Object.create(RSA.prototype));
	...
	reportFailure(test) {
		...
		this.exec = this.sandbox
			.stub(cp, 'exec')
			.callsFake(function (command, callback) {
				callback('some error');
			});
...
exports['Tessel.prototype.provision'] = {
	setUp(done) {
		...
		this.writeFile = this.sandbox
			.stub(fs, 'writeFile')
			.callsFake((dirname, contents, options, callback) => {
				callback(null);
			});
		this.ensureDir = this.sandbox
			.stub(fs, 'ensureDir')
	...
	alreadyAuthedError(test) {
		...
		this.setupLocal = this.sandbox
			.stub(provision, 'setupLocal')
			.callsFake(function () {
				return Promise.resolve();
			});
		this.authTessel = this.sandbox
			.stub(provision, 'authTessel')
			.callsFake(function () {
		...
		this.tessel
			...
			.then(() => {
				test.equal(
					...
					'Tessel is already authenticated with this computer.',
				);
				test.done();
			})
			.catch(() => {
				test.ok(
					false,
					'The AlreadyAuthenticatedError will resolve, not reject.',
				);
				test.done();
			});
	},
	...
	keysCreatedCorrectPermissions(test) {
		...
		provision
			.setupLocal(/* intentionally empty */)
			.then(() => {
	...
	keyAlreadyInRemoteAuth(test) {
		...
		createTestKeys((error) => {
			if (error) {
				...
				provision
					.authTessel(this.tessel, testPath)
					.then(() => {
						test.ok(
							false,
							'Authorize did not return fail even though remote Tessel already listed user key.',
						);
						test.done();
					})
					.catch((error) => {
			...
			if (error) {
				...
				provision
					...
					.catch(function unexpectedRoute() {
						test.ok(
							false,
							'Provision did not copy keys to remote device when it should have.',
						);
						test.done();
					});
...
exports['provision.setDefaultKey'] = {
	...
	failNoKey(test) {
		...
		provision
			...
			.then(function noCall() {
				...
				test.ok(
					false,
					'provision.setDefaultKey should not have been successful',
				);
				test.done();
			})
	...
	failWrongType(test) {
		...
		provision
			.setDefaultKey(path.parse('~/.tessel/id_rsa'))
			.then(function noCall() {
				// This test should throw an error. Fail if it didn't
				test.ok(
					false,
...
function createTestKeys(callback) {
	...
	createKeyTestFolder(function createKeys(err) {
		...
		async.parallel(
			[
				(next) => fs.writeFile(testPath + '.pub', fakeKeyFileData, next),
				(next) => fs.writeFile(testPath, fakeKeyFileData, next),
			],
			callback,
		);
	});

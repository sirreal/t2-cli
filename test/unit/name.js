...
exports['Tessel.prototype.rename'] = {
	setUp(done) {
		...
		this.getMACAddress = this.sandbox
			.stub(Tessel.prototype, 'getMACAddress')
			.callsFake(() => {
				return Promise.resolve('TheFakeMACAddress');
			});
		this.resetMDNS = this.sandbox
			.stub(Tessel.prototype, 'resetMDNS')
			.callsFake(() => {
	...
	renameInvalid(test) {
		...
		this.rename({
			...
			test.equal(
				err,
				'Invalid name: !@#$. The name must be a valid hostname string. See http://en.wikipedia.org/wiki/Hostname#Restrictions_on_valid_host_names.',
			);
			test.equal(this.closeTesselConnections.callCount, 0);
			test.done();
		});
	...
	resetName(test) {
		...
		this.tessel
			...
			.then(() => {
				...
				test.ok(
					this.setHostname.lastCall.calledWith('Tessel-TheFakeMACAddress'),
				);

...
exports['Tessel.prototype.memoryInfo'] = {
	setUp: function (done) {
		this.simpleExec = sandbox
			.stub(Tessel.prototype, 'simpleExec')
			.callsFake(() => Promise.resolve(meminfo));
	...
	meminfo: function (test) {
		...
		this.tessel.memoryInfo().then((memory) => {
			...
			test.deepEqual(this.simpleExec.lastCall.args[0], [
				'cat',
				'/proc/meminfo',
			]);
...
exports['Tessel.prototype.deploy'] = {
	setUp: function (done) {
		...
		this.tarBundle = sandbox
			...
			.returns(Promise.resolve(jsCodeReference));
	...
	resolveLanguageOrRuntime: function (test) {
		...
		this.simpleExec = sandbox
			.stub(Tessel.prototype, 'simpleExec')
			.callsFake(() => Promise.reject('Bailout'));
		...
		Promise.all([
			...
			test.deepEqual(
				this.resolveLanguage.getCall(0).returnValue,
				deployment.js,
			);
			test.deepEqual(
				this.resolveLanguage.getCall(1).returnValue,
				deployment.js,
			);
			test.deepEqual(
				this.resolveLanguage.getCall(2).returnValue,
				deployment.js,
			);
			test.deepEqual(
				this.resolveLanguage.getCall(3).returnValue,
				deployment.rs,
			);
			test.deepEqual(
				this.resolveLanguage.getCall(4).returnValue,
				deployment.rs,
			);
			test.deepEqual(
				this.resolveLanguage.getCall(5).returnValue,
				deployment.rs,
			);
			test.deepEqual(
				this.resolveLanguage.getCall(6).returnValue,
				deployment.py,
			);
			test.deepEqual(
				this.resolveLanguage.getCall(7).returnValue,
				deployment.py,
			);
			test.deepEqual(
				this.resolveLanguage.getCall(8).returnValue,
				deployment.py,
			);
	...
	push: function (test) {
		...
		deployTestCode(
			...
			(error) => {
				...
				test.equal(
					this.createShellScript.lastCall.args[1].resolvedEntryPoint,
					expectedPath,
				);
...
exports['Tessel.prototype.restart'] = {
	...
	restartNonExistent: function (test) {
		...
		this.tessel.restart(opts).catch((error) => {
			test.equal(error, '"index.js" not found on undefined');
			test.done();
		});

		setImmediate(() => {
			this.tessel._rps.stderr.emit(
				'data',
				Buffer.from('No such file or directory'),
			);
			this.tessel._rps.emit('close');
		});
	},
...
exports['deploy.start'] = {
	...
	start: function (test) {
		...
		deploy.start(this.tessel, 'index.js').then(() => {
			test.equal(this.simpleExec.callCount, 3);
			test.deepEqual(this.simpleExec.firstCall.args[0], [
				'mv',
				'/tmp/remote-script/',
				'/app/',
			]);
			test.deepEqual(this.simpleExec.secondCall.args[0], [
				'/etc/init.d/tessel-app',
				'enable',
			]);
			test.deepEqual(this.simpleExec.thirdCall.args[0], [
				'/etc/init.d/tessel-app',
				'start',
			]);
			test.done();
...
exports['deploy.run'] = {
	...
	runResolveEntryPoint(test) {
		...
		this.exec = sandbox
			...
			.callsFake((command, opts, callback) => {
		...
		deploy
			...
			.then(() => {
				test.deepEqual(this.exec.lastCall.args[0], [
					'node',
					'/tmp/remote-script/foo',
					'--key=value',
				]);
				test.done();
			});
		...
		this.exec = sandbox
			...
			.callsFake((command, opts, callback) => {
				...
				if (this.exec.callCount === 1) {
					test.deepEqual(command, [
						'chmod',
						'+x',
						'/tmp/remote-script/' + entryPoint,
					]);
				}
	...
	runPostRunExistsLAN(test) {
		...
		this.postRun = sandbox
			.stub(deployment.js, 'postRun')
			.callsFake(() => Promise.resolve());
		this.exec = sandbox
			.stub(this.tessel.connection, 'exec')
...
exports['Tessel.REMOTE_*_PATH'] = {
	expectedPaths: function (test) {
		...
		test.equal(
			...
			path.posix.join(Tessel.REMOTE_TMP_PATH, Tessel.REMOTE_SCRIPT_PATH),
		);

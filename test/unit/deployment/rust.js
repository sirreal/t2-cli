...
exports['deploy.rust'] = {
	...
	remoteRustCompilationSuccess(test) {
		...
		deployment.rs
			...
			.then(() => {
				...
				test.equal(
					this.httpRequest.lastCall.args[0].path,
					'/rust-compile?target=charmander',
				);
	...
	remoteRustCompilationMissingBinary(test) {
		...
		deployment.rs
			...
			.catch((error) => {
				test.equal(
					error.toString(),
					'Error: Neither binary nor error returned by cross compilation server.',
				);
				test.done();
			});
	...
	rustPreBundle(test) {
		...
		this.checkBinaryName = sandbox
			.stub(rust, 'checkBinaryName')
			.returns(Promise.resolve());
	...
	rustTarBundleLocal(test) {
		...
		this.remoteRustCompilation = sandbox
			.stub(deployment.rs, 'remoteRustCompilation')
			.callsFake(() => Promise.resolve(null));

		this.runBuild = sandbox
			.stub(rust, 'runBuild')
			.callsFake(() => Promise.resolve(__filename));
	...
	rustTarBundleRemote(test) {
		...
		this.remoteRustCompilation = sandbox
			.stub(deployment.rs, 'remoteRustCompilation')
			.callsFake(() => Promise.resolve(Buffer.from([])));
	...
	checkBinaryNames(test) {
		...
		this.cargoMetadata = sandbox.stub(rust, 'cargoMetadata').callsFake(() =>
			Promise.resolve({
				packages: [
					{
						name: 'blinky',

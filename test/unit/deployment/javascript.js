...
var listRuleLength =
	lists.includes.length + lists.ignores.length + lists.disallowed.length;
...
exports['Deployment: JavaScript'] = {
	setUp(done) {
		...
		this.injectBinaryModules = sandbox
			.stub(deployment.js, 'injectBinaryModules')
			.callsFake(() => Promise.resolve());
		this.resolveBinaryModules = sandbox
			.stub(deployment.js, 'resolveBinaryModules')
			.callsFake(() => Promise.resolve());
		this.tarBundle = sandbox
			.stub(deployment.js, 'tarBundle')
			.callsFake(() => Promise.resolve(reference));
		...
		this.fetchCurrentBuildInfo = sandbox
			.stub(this.tessel, 'fetchCurrentBuildInfo')
			.returns(Promise.resolve('40b2b46a62a34b5a26170c75f7e717cea673d1eb'));
		this.fetchNodeProcessVersions = sandbox
			.stub(this.tessel, 'fetchNodeProcessVersions')
			.returns(Promise.resolve(processVersions));
		this.requestBuildList = sandbox
			.stub(updates, 'requestBuildList')
			.returns(Promise.resolve(tesselBuilds));

		this.pWrite = sandbox.stub(Preferences, 'write').returns(Promise.resolve());
		this.exists = sandbox
			.stub(fs, 'exists')
			.callsFake((fpath, callback) => callback(true));

		this.spinnerStart = sandbox.stub(log.spinner, 'start');
		this.spinnerStop = sandbox.stub(log.spinner, 'stop');

		deleteTemporaryDeployCode().then(done);
	},
...
exec node --harmony /app/remote-script/index.js --key=value`;
		...
		this.end.restore();
		this.end = sandbox
			.stub(this.tessel._rps.stdin, 'end')
			.callsFake((buffer) => {
			...
			this.tessel
				...
				.catch(function (err) {
					test.equal(
						err,
						undefined,
						'We hit a catch statement that we should not have.',
					);
				});
		});
	},
...
exports['deployment.js.compress with uglify.es.minify()'] = {
	...
	minifyFromBuffer(test) {
		test.expect(1);
		test.equal(
			deployment.js.compress('es', Buffer.from(codeContents)),
			codeContents,
		);
		test.done();
	},
...
exports['deployment.js.compress with uglify.js.minify()'] = {
	...
	minifyFromBuffer(test) {
		test.expect(1);
		test.equal(
			deployment.js.compress('js', Buffer.from(codeContents)),
			codeContents,
		);
		test.done();
	},
...
exports['deployment.js.tarBundle'] = {
	...
	actionsGlobFilesNested(test) {
		...
		test.deepEqual(files, ['.tesselignore', 'nested/.tesselignore']);
     ...
     */
		...
		deployment.js
			...
			.then(() => {
				test.equal(
					path.relative.firstCall.args[0],
					path.normalize('test/unit/fixtures/project'),
				);
				test.equal(
					path.relative.firstCall.args[1],
					path.normalize('test/unit/fixtures/project'),
				);
				test.done();
			});
	...
	slimHitsErrorFromFstreamReader(test) {
		...
		// Need to stub function in _actual_ fs (but we use fs-extra)
		this.readerPipe = sandbox
			.stub(fstream.Reader.prototype, 'pipe')
			.callsFake(function () {
	...
	slimHitsErrorFromProjectCollect(test) {
		...
		this.collect = sandbox
			.stub(Project.prototype, 'collect')
			.callsFake((handler) => {
				handler(new Error('foo'));
			});
		...
		deployment.js
			...
			.then((bundle) => {
				extract(bundle, (error, entries) => {
					...
					test.deepEqual(entries, ['arrow.js', 'index.js', 'package.json']);
					test.done();
				});
			})
			.catch(() => {
     ...
     */
		...
		deployment.js
			...
			.then((bundle) => {
				...
				test.equal(
					minified,
					'const e=require("tessel"),{2:o,3:l}=e.led;o.on(),setInterval(()=>{o.toggle(),l.toggle()},100),console.log("I\'m blinking! (Press CTRL + C to stop)");',
				);
    ...
    */
		...
		deployment.js
			...
			.then(() => {
				...
				test.equal(
					this.warn.firstCall.args[0],
					'Some assets in this project were not deployed (see: t2 run --help)',
				);
	...
	detectAssetsWithoutIncludeEliminatedByDepGraph(test) {
		...
		const entryPoint = 'index.js';
		const target =
			'test/unit/fixtures/project-assets-without-include-eliminated-by-dep-graph';
    ...
    */
	...
	iteratesBinaryModulesUsed(test) {
		...
		this.forEach = sandbox
			.stub(Map.prototype, 'forEach')
			.callsFake((handler) => {
				handler(details);
			});
		...
		deployment.js
			...
			.then(() => {
				...
				test.deepEqual(this.minimatch.getCall(0).args, [
					'',
					'a',
					{
						matchBase: true,
				...
				test.deepEqual(this.minimatch.getCall(1).args, [
					'',
					'b',
					{
						matchBase: true,
				...
				test.deepEqual(this.minimatch.getCall(2).args, [
					'',
					'node_modules/**/tessel/**/*',
					{
						matchBase: true,
...
exports['deploy.findProject'] = {
	...
	noPackageJsonSingle(test) {
		...
		var entryPoint = path.normalize(
			'test/unit/fixtures/project-no-package.json/index.js',
		);
	...
	noPackageJsonUseProgramDirname(test) {
		...
		deploy.findProject(opts).then((project) => {
			test.equal(
				project.pushdir,
				path.dirname(path.join(process.cwd(), entryPoint)),
			);
			test.done();
		});
	},
...
exports['deploy.sendBundle, error handling'] = {
	...
	findProject(test) {
		...
		this.findProject = sandbox
			.stub(deploy, 'findProject')
			.callsFake(() => Promise.reject(this.failure));

		deploy
			.sendBundle(this.tessel, {
				lang: deployment.js,
			})
			.catch((error) => {
	...
	resolveBinaryModules(test) {
		...
		this.findProject = sandbox.stub(deploy, 'findProject').callsFake(() =>
			Promise.resolve({
				pushdir: '',
				entryPoint: '',
			}),
...
exports['deployment.js.preBundle'] = {
	setUp(done) {
		...
		this.receive = sandbox
			.stub(this.tessel, 'receive')
			.callsFake((rps, callback) => {
				rps.emit('close');
				callback();
			});
		...
		this.findProject = sandbox.stub(deploy, 'findProject').returns(
			Promise.resolve({
				pushdir: '',
				entryPoint: '',
			}),
		...
		this.tarBundle = sandbox
			.stub(deployment.js, 'tarBundle')
			.returns(Promise.resolve(Buffer.from([0x00])));
		this.pathResolve = sandbox.stub(path, 'resolve');
    ...
    Once complete, retry:`;
		...
		this.exists = sandbox
			.stub(fs, 'exists')
			.callsFake((fpath, callback) => callback(false));
...
exports['deployment.js.resolveBinaryModules'] = {
	setUp(done) {
		...
		this.readGypFileSync = sandbox
			.stub(deployment.js.resolveBinaryModules, 'readGypFileSync')
			.callsFake(() => {
	...
	bailOnSkipBinary(test) {
		...
		deployment.js
			...
			.then(() => {
				...
				//
				test.equal(
					this.exists.lastCall.args[0].endsWith(
						path.normalize(
			...
			.catch((error) => {
				...
				test.ok(
					error
						.toString()
						.includes(
							`Error: Cannot find module '${path.normalize('node_modules/release/package.json')}'`,
						),
				);
				test.done();
			});
	...
	findsModulesMissingBinaryNodeFiles(test) {
		...
		deployment.js
			...
			.then(() => {
				test.deepEqual(this.globFiles.lastCall.args[1], [
					'node_modules/**/*.node',
	...
	spawnPythonScriptReturnsNull(test) {
		...
		this.readGypFileSync.restore();
		this.readGypFileSync = sandbox.spy(
	...
	spawnPythonScript(test) {
		...
		this.spawnSync = sandbox.stub(cp, 'spawnSync').callsFake(() => {
			return {
				output: [
					null,
					Buffer.from(
						'{"targets": [{"target_name": "missing","sources": ["capture.c", "missing.cc"]}]}',
						'utf8',
					),
				],
			};
		...
		deployment.js
			...
			.then(() => {
				...
				test.equal(
					python.startsWith(
						'import ast, json; print json.dumps(ast.literal_eval(open(',
					),
					true,
				);
	...
	failsWithMessage(test) {
		...
		this.globSync.restore();
		this.globSync = sandbox.stub(glob, 'sync').callsFake(() => {
			return [path.normalize('node_modules/missing/binding.gyp')];
		});
		this.readGypFileSync.restore();
		this.readGypFileSync = sandbox
			.stub(deployment.js.resolveBinaryModules, 'readGypFileSync')
			...
			.then(() => {
				...
				cachedBinaryPaths.forEach((cbp, callIndex) => {
					test.equal(
						this.exists
							.getCall(callIndex)
							.args[0].endsWith(path.normalize(cbp)),
						true,
					);
				});
	...
	requestsRemote(test) {
		...
		deployment.js
			...
			.then(() => {
				...
				test.equal(
					this.mkdirp.lastCall.args[0].endsWith(
						path.normalize(
							'.tessel/binaries/release-1.1.1-Release-node-v46-linux-mipsel',
						),
					),
				...
				test.equal(
					requestArgs.url,
					'http://packages.tessel.io/npm/release-1.1.1-Release-node-v46-linux-mipsel.tgz',
				);
	...
	requestsRemoteGunzipErrors(test) {
		...
		deployment.js
			...
			.then(() => {
				...
				test.equal(
					this.removeSync.lastCall.args[0].endsWith(
						path.normalize(
							'.tessel/binaries/release-1.1.1-Release-node-v46-linux-mipsel',
						),
					),
...
exports['deployment.js.injectBinaryModules'] = {
	...
	copies(test) {
		...
		deployment.js
			...
			.then(() => {
				deployment.js
					.injectBinaryModules(this.globRoot, fsTemp.mkdirSync(), {})
					.then(() => {
        ...
        */
						...
						test.equal(
							args[0][0].endsWith(
								path.normalize(
									'debug-1.1.1-Debug-node-v46-linux-mipsel/Debug/debug.node',
								),
							),
							true,
						);
						test.equal(
							args[0][1].endsWith(
								path.normalize('debug/build/Debug/debug.node'),
							),
							true,
						);
						...
						test.equal(
							args[2][1].endsWith(
								path.normalize('linked/build/bindings/linked.node'),
							),
							true,
						);
						...
						test.equal(
							args[4][1].endsWith(
								path.normalize('missing/build/Release/missing.node'),
							),
							true,
						);
						...
						test.equal(
							args[6][1].endsWith(
								path.normalize('release/build/Release/release.node'),
							),
							true,
						);
	...
	usesBinaryHighestInTreeWhenEncounteringDuplicates(test) {
		...
		this.relative = sandbox.stub(path, 'relative').callsFake(() => {
			return path.join(
				FIXTURE_PATH,
				'/project-binary-modules-duplicate-lower-deps/',
			);
		});
	...
	rewriteBinaryBuildPlatformPaths(test) {
			...
			.callsFake((handler) => {
				handler({
					...
					buildPath: path.normalize(
						'/build/Release/node-v46-FAKE_PLATFORM-FAKE_ARCH/',
					),
					buildType: 'Release',
					globPath: path.normalize(
						'node_modules/serialport/build/Release/node-v46-FAKE_PLATFORM-FAKE_ARCH/serialport.node',
					),
					ignored: false,
					name: 'serialport',
					...
					resolved: true,
					version: '2.0.6',
					extractPath: path.normalize(
						'~/.tessel/binaries/serialport-2.0.6-Release-node-v46-linux-mipsel',
					),
				});
			});
		...
		deployment.js
			...
			.then(() => {
				...
				test.equal(
					this.copySync.firstCall.args[0].endsWith(
						path.normalize('linux-mipsel/serialport.node'),
					),
					true,
				);
				test.equal(
					this.copySync.firstCall.args[1].endsWith(
						path.normalize('linux-mipsel/serialport.node'),
					),
					true,
				);
		...
		this.forEach = sandbox
			...
			.callsFake((handler) => {
				handler({
					...
					buildType: 'Release',
					globPath: path.normalize(
						'node_modules/sqlite3/lib/binding/node-v46-something-else/node_sqlite3.node',
					),
					ignored: false,
					name: 'sqlite3',
					...
					version: '3.1.4',
					extractPath: path.normalize(
						'~/.tessel/binaries/sqlite3-3.1.4-Release',
					),
				});
			});
			...
			.then(() => {
				...
				test.equal(
					this.copySync
						.getCall(0)
						.args[0].endsWith(
							path.normalize('node-v46-something-else/node_sqlite3.node'),
						),
					true,
				);
				// OUR PATH
				test.equal(
					this.copySync
						.getCall(1)
						.args[0].endsWith(path.normalize('Release/node_sqlite3.node')),
					true,
				);
				test.done();
			});
	...
	tryCatchTwiceAndFailGracefullyWithMissingBinaryMessage(test) {
		...
		this.forEach = sandbox
			...
			.callsFake((handler) => {
				handler({
					...
					buildType: 'Release',
					globPath: path.normalize(
						'node_modules/not-a-thing/lib/binding/node-v46-something-else/not-a-thing.node',
					),
					ignored: false,
					name: 'not-a-thing',
					...
					version: '3.1.4',
					extractPath: path.normalize(
						'~/.tessel/binaries/not-a-thing-3.1.4-Release',
					),
				});
			});

		this.error = sandbox.stub(log, 'error');
		this.logMissingBinaryModuleWarning = sandbox.stub(
			deployment.js,
			'logMissingBinaryModuleWarning',
		);
		...
		deployment.js
			...
			.then(() => {
				...
				test.equal(
					String(this.error.lastCall.args[0]).includes('E_THIS_IS_NOT_REAL'),
					true,
				);
				test.done();
			});
...
exports['deployment.js.lists'] = {
	...
	checkIgnores(test) {
		...
		var ignores = ['node_modules/**/tessel/**/*'];
...
exports['deployment.js.logMissingBinaryModuleWarning'] = {
	setUp(done) {
		...
		this.details = {
			...
			buildType: 'Release',
			globPath: path.normalize(
				'node_modules/compiled-binary/build/Release/node-v46-FAKE_PLATFORM-FAKE_ARCH/compiled-binary.node',
			),
			ignored: false,
			...
			version: '2.0.6',
			extractPath: path.normalize(
				'~/.tessel/binaries/compiled-binary-2.0.6-Release-node-v46-linux-mipsel',
			),
		};
		done();
	},
	...
	includesModuleNameAndVersion(test) {
		...
		test.equal(
			output.includes('Pre-compiled module is missing: compiled-binary@2.0.6'),
			true,
		);
		test.done();
	},

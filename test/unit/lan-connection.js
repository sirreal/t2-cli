...
exports['LAN.Connection'] = {
	setUp(done) {
		...
		this.readFileSync = this.sandbox
...
exports['LAN.Connection.prototype.exec'] = {
	...
	closed(test) {
		...
		this.lanConnection.exec(undefined, (error) => {
			test.equal(
				error.message,
				'Remote SSH connection has already been closed',
			);
			test.done();
		});
	},
...
exports['LAN.Scanner.prototype.start'] = {
	setUp(done) {
		this.sandbox = sinon.sandbox.create();
		this.setImmediate = this.sandbox
			.stub(global, 'setImmediate')
			.callsFake((cb) => cb());
		this.createBrowser = this.sandbox
			.stub(mdns, 'createBrowser')
			.callsFake(() => {
		...
		this.readFileSync = this.sandbox
			.stub(fs, 'readFileSync')
			.callsFake(() => 'this is the contents of id_rsa');
	...
	updateDiscoveredThrows(test) {
		...
		this.LANConnection = this.sandbox
			.stub(LAN, 'Connection')
			.callsFake(function () {
				throw new Error('get outta here!');
			});

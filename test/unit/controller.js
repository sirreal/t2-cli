...
exports['controller.provision'] = {
	setUp(done) {
		...
		this.basic = this.sandbox.stub(log, 'basic');
		this.error = this.sandbox.stub(log, 'error');
		this.isProvisioned = this.sandbox
			.stub(Tessel, 'isProvisioned')
			.returns(true);
		this.standardTesselCommand = this.sandbox
			.stub(controller, 'standardTesselCommand')
...
exports['controller.closeTesselConnections'] = {
	...
	rejectsOnError(test) {
		...
		this.asyncEach = this.sandbox
			.stub(async, 'each')
			.callsFake((tessels, next, done) => {
				done(new Error('This is only a test'));
			});

		controller.closeTesselConnections([1, 2, 3]).catch((error) => {
	...
	callsCloseOnAllAuthorizedLANConnections(test) {
		...
		controller.closeTesselConnections([a, b, c]).then(() => {
	...
	resolvesForUnauthorizedLANConnections(test) {
		...
		controller.closeTesselConnections([a]).then(() => {
...
exports['controller.runHeuristics'] = {
	...
	oneUSBDevice(test) {
		...
		controller.runHeuristics({}, [a]).then(function (tessel) {
	...
	bothConnectionsAndLAN(test) {
		...
		controller.runHeuristics({}, [a, b]).then(function (tessel) {
			test.deepEqual(b, tessel);
			test.done();
		});
	},
	...
	catchAmbiguityTwoLAN(test) {
		...
		controller
			...
			.then(() => {
				test.ok(
					false,
					'Test failed because expected HeuristicAmbiguityError did not occur.',
				);
				test.done();
			})
			.catch((error) => {
...
exports['controller.envVersions'] = {
	setUp(done) {
		...
		this.fetchNodeProcessVersion = this.sandbox
			.stub(Tessel.prototype, 'fetchNodeProcessVersion')
			.callsFake(() => {
	...
	properVersionsReturned(test) {
		...
		controller.envVersions(opts).then(() => {
			...
			test.equal(
				this.info.getCall(1).args[0],
				`t2-cli: ${this.packageJsonVersion}`,
			);
...
exports['Tessel.list'] = {
	setUp(done) {
		...
		this.seeker = this.sandbox
			.stub(discover, 'TesselSeeker')
			.callsFake(function () {
	...
	oneUSBTessel(test) {
		...
		Tessel.list(this.standardOpts).then(() => {
	...
	runHeuristicsRejection(test) {
		...
		this.runHeuristics = this.sandbox
			.stub(controller, 'runHeuristics')
			.returns(Promise.reject(new Error('Bogus error')));
		this.closeTesselConnections = this.sandbox
			.stub(controller, 'closeTesselConnections')
			.returns(Promise.resolve());
...
exports['Tessel.get'] = {
	setUp(done) {
		...
		this.closeTesselConnections = this.sandbox.stub(
			controller,
			'closeTesselConnections',
		);
	...
	oneUnNamedTessel(test) {
		...
		Tessel.get(this.standardOpts).then(() => {
		...
		this.runHeuristics = this.sandbox
			...
			.callsFake(() => {
				return Promise.reject(
					new controller.HeuristicAmbiguityError('Bogus error'),
				);
			});
		this.menuPrompt = this.sandbox.stub(Menu, 'prompt').callsFake((prompt) => {
			var value = prompt.translate('\tUSB\ta\t');
	...
	standardCommandNoTessels(test) {
		...
		controller
			.standardTesselCommand(this.standardOpts, function () {
	...
	standardCommandSuccess(test) {
		...
		controller
			.standardTesselCommand(this.standardOpts, (tessel) => {
	...
	standardCommandFailed(test) {
		...
		controller
			.standardTesselCommand(this.standardOpts, function (tessel) {
				// Make sure we have been given the tessel that was emitted
				test.deepEqual(tessel, usb);
				// Finish the command
			...
			.then(() => {
				test.ok(
					false,
					'standardTesselCommand was expected to fail, but did not.',
				);
				test.done();
			})
			.catch((error) => {
	...
	invalidAccessPointPSKASCIIPasswordTooLong(test) {
		...
		controller
			.createAccessPoint({
				ssid: 'test',
				password:
					'this is a very long passphrase. in fact, it is over 63 characters, which makes it invalid.',
				security: 'psk',
			})
			.catch((error) => {
				test.ok(error);
...
exports['controller.getWifiInfo'] = {
	...
	requestNetworkInfo(test) {
		...
		this.sandbox
			.stub(controller, 'standardTesselCommand')
			.callsFake((options, callback) => callback(this.lanbot));

		this.sandbox
			.stub(this.lanbot, 'getWifiInfo')
			.returns(Promise.resolve(global.networkInfo));

		controller.getWifiInfo(options).then(() => {
			test.equal(this.info.callCount, 4);
...
exports['controller.printAvailableNetworks'] = {
	...
	listCountOfVisibleToTesselSingle(test) {
		...
		this.findAvailableNetworks = this.sandbox
			.stub(this.tessel, 'findAvailableNetworks')
			.callsFake(() => {
				return Promise.resolve([
					{
						ssid: 'foo',
		...
		controller
			...
			.then(() => {
				...
				test.equal(
					this.info.firstCall.args[0],
					'Scanning for visible networks...',
				);
				test.equal(
					this.info.lastCall.args[0],
					'Found 1 network visible to robocop:',
				);
	...
	listCountOfVisibleToTesselPlural(test) {
		...
		controller
			...
			.then(() => {
				...
				test.equal(
					this.info.lastCall.args[0],
					'Found 2 networks visible to robocop:',
				);
	...
	listCountOfVisibleToTesselPluralZero(test) {
		...
		controller
			...
			.then(() => {
				...
				test.equal(
					this.info.lastCall.args[0],
					'Found 0 networks visible to robocop:',
				);
				test.done();
			})
			.catch((error) => {
...
exports['controller.uninstaller'] = {
	setUp(done) {
		...
		this.homedir = this.sandbox
			.stub(installer, 'homedir')
			.returns(Promise.resolve());
		done();
	},
...
exports['controller.updateWithRemoteBuilds'] = {
	...
	noUsbRejection(test) {
		...
		controller.updateWithRemoteBuilds({}, this.tessel).catch((error) => {
			test.equal(
				error.toString(),
	...
	noForceRejection(test) {
		...
		this.fetchCurrentBuildInfo = this.sandbox
			.stub(this.tessel, 'fetchCurrentBuildInfo')
			.returns(Promise.reject(new Error('No such file or directory')));

		controller
			.updateWithRemoteBuilds(
				{
					force: false,
				},
				this.tessel,
			...
			.catch((error) => {
				...
				test.equal(
					this.warn.lastCall.args[0].includes(
						'Could not find firmware version on ',
					),
					true,
				);
				test.done();
...
exports['controller.restore'] = {
	setUp(done) {
		...
		this.get = this.sandbox
			.stub(controller, 'get')
			.returns(Promise.resolve(this.tessel));
		this.tesselRestore = this.sandbox
			.stub(this.tessel, 'restore')
			.returns(Promise.resolve());
	...
	restoreCallPipelineRejected(test) {
		...
		this.tesselRestore = this.sandbox
			.stub(this.tessel, 'restore')
			.returns(Promise.reject(new Error('whatever')));
...
exports['controller.deploy'] = {
	setUp(done) {
		...
		this.deploy = this.sandbox
			.stub(this.tessel, 'deploy')
			.returns(Promise.resolve());
	...
	deployCallPipeline(test) {
		...
		controller.deploy(options).then(() => {
...
exports['controller.restart'] = {
	setUp(done) {
		...
		this.restart = this.sandbox
			.stub(this.tessel, 'restart')
			.returns(Promise.resolve());
	...
	restartCallPipeline(test) {
		...
		controller.restart(options).then(() => {
			test.equal(this.standardTesselCommand.lastCall.args[0], options);
...
exports['controller.eraseScript'] = {
	setUp(done) {
		...
		this.eraseScript = this.sandbox
			.stub(this.tessel, 'eraseScript')
			.returns(Promise.resolve());
	...
	eraseScriptCallPipeline(test) {
		...
		controller.eraseScript(options).then(() => {
			test.equal(this.standardTesselCommand.lastCall.args[0], options);
...
exports['controller.setWiFiState'] = {
	setUp(done) {
		...
		this.setWiFiState = this.sandbox
			.stub(this.tessel, 'setWiFiState')
			.returns(Promise.resolve());
	...
	setWiFiStateCallPipeline(test) {
		...
		controller.setWiFiState(options).then(() => {
			test.equal(this.standardTesselCommand.lastCall.args[0], options);
	...
	setWiFiStateMissingEnable(test) {
		...
		controller
			...
			.catch((error) => {
				test.equal(
					error.toString(),
					'Error: Missing Wifi State: property "enable" not provided.',
				);
				test.done();
			});
	},
...
exports['controller.disableAccessPoint'] = {
	setUp(done) {
		...
		this.disableAccessPoint = this.sandbox
			.stub(this.tessel, 'disableAccessPoint')
			.returns(Promise.resolve());
	...
	disableAccessPointCallPipeline(test) {
		...
		controller.disableAccessPoint(options).then(() => {
			test.equal(this.standardTesselCommand.lastCall.args[0], options);
...
exports['controller.enableAccessPoint'] = {
	setUp(done) {
		...
		this.enableAccessPoint = this.sandbox
			.stub(this.tessel, 'enableAccessPoint')
			.returns(Promise.resolve());
	...
	enableAccessPointCallPipeline(test) {
		...
		controller.enableAccessPoint(options).then(() => {
			test.equal(this.standardTesselCommand.lastCall.args[0], options);
...
exports['controller.rename'] = {
	setUp(done) {
		...
		this.rename = this.sandbox
			.stub(this.tessel, 'rename')
			.returns(Promise.resolve());
	...
	willNotRenameSameAsCurrent(test) {
		...
		controller.rename(options).then(() => {
			// Assert that a warning was logged when
			// the new name is the same as the old name
			test.equal(this.warn.callCount, 1);
			test.equal(
				this.warn.lastCall.args[0],
				'Name of device is already TestTessel',
			);
			test.done();
		});
	},

...
module.exports = (grunt) => {
	grunt.initConfig({
		nodeunit: {
			tests: ['test/unit/*.js', 'test/unit/deployment/*.js'],
		},
		jshint: {
			all: {
				options: {
		...
		jsbeautifier: {
			all: ['bin/*', 'lib/**/*.js', 'test/**/*.js', 'Gruntfile.js'],
			options: {
				js: {
					braceStyle: 'collapse',
	...
	grunt.registerTask(
		'nodeunit:only',
		'Run a single test specified by a target; usage: "grunt nodeunit:only:<module-name>[.js]"',
		(file) => {
			if (file) {
	...
	grunt.registerTask(
		'nodeunit:file',
		'Run a subset of tests by specifying a file name or glob expression. Usage: "grunt nodeunit:file:<file.ext>" or "grunt nodeunit:file:<expr>"',
		(input) => {
			const config = ['test/common/bootstrap.js'];
	...
	grunt.registerTask(
		'nodeunit:files',
		'Run a subset of tests by specifying a file name, bracket list of file names, or glob expression. Usage: "grunt nodeunit:file:<file.ext>" or "grunt nodeunit:file:<expr>"',
		(file) => {
	...
	grunt.registerTask(
		'changelog',
		'"changelog", "changelog:v0.0.0..v0.0.2" or "changelog:v0.0.0"',
		(arg) => {
			...
			cp.exec(
				`git log --format='|%h|%s|' ${revisionRange}`,
				(error, result) => {
					if (error) {
